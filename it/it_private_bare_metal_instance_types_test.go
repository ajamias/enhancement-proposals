...
var _ = Describe("Private bare metal instance types", func() {
	...
	It("Rejects update of immutable CPU cores", func() {
		...
		Expect(status.Message()).To(ContainSubstring("spec.hardware.cpu.cores"))
	...
	It("Rejects update of immutable CPU architecture", func() {
		...
		Expect(ok).To(BeTrue())
		Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
		Expect(status.Message()).To(ContainSubstring("spec.hardware.cpu.architecture"))
		Expect(status.Message()).To(ContainSubstring("immutable"))
	})
	...
	It("Rejects update of immutable memory total", func() {
		...
		Expect(ok).To(BeTrue())
		Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
		Expect(status.Message()).To(ContainSubstring("spec.hardware.memory.total_gb"))
		Expect(status.Message()).To(ContainSubstring("immutable"))
	})

	It("Allows deletion when no bare metal instances reference the type", func() {
